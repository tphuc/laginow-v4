...
import { updateMarketingPosts } from "./actions"





export function UpdateMarketingPosts({ defaultValue, allPosts }: { defaultValue?: any[], allPosts: any[] }) {
    ...
    async function onSubmit(values) {
        ...
        try {
            ...
            let res = await updateMarketingPosts(values?.posts?.map((item => ({
                id: item?.value
            }))))
        ...
        <Form  {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2 md:space-y-4">
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            <FormLabel>Bài viết hiển thị trang đầu (*)</FormLabel>
                            <FormControl>
                                <MultiSelect
