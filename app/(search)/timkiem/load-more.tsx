...
type loadMoreAction<T extends string | number = any> = T extends number
  ? (offset: T) => Promise<readonly [React.ReactElement, number | null]>
  : T extends string
  ? (offset: T) => Promise<readonly [React.ReactElement, string | null]>
  : any;
...
const LoadMore = <T extends string | number = any>({
  ...
  const [loadMoreNodes, setLoadMoreNodes] = React.useState<React.ReactElement[]>(
    []
  );
